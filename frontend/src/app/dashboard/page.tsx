...
function StatCard({ icon: Icon, label, value, color, sublabel }: {
  icon: React.ElementType; label: string; value: number | string; color: string; sublabel?: string;
}) {
  ...
  return (
    <div className="cyber-card p-5">
      ...
      <div className="text-text-secondary text-xs font-mono uppercase tracking-wider">{label}</div>
      {sublabel && <div className="text-text-secondary text-xs mt-1 opacity-60">{sublabel}</div>}
    </div>
  );
}
...
export default function DashboardPage() {
  ...
  const pieData = stats ? [
    { name: "Safe", value: stats.safe_scans || 0, color: "#00ff88" },
    ...
    <div className="p-8">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-neon-cyan font-mono text-xs uppercase tracking-widest">Live Dashboard</span>
        </div>
        <h1 className="font-display text-2xl font-bold text-text-primary">
          Welcome back, <span className="text-neon-cyan">{user?.username}</span>
        </h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <StatCard icon={Shield} label="Total Scans" value={isLoading ? "—" : (stats?.total_scans ?? 0)} color="cyan" />
        <StatCard icon={AlertTriangle} label="Threats Detected" value={isLoading ? "—" : (stats?.threats_detected ?? 0)} color="red" sublabel={`${stats?.threat_rate ?? 0}% threat rate`} />
        <StatCard icon={CheckCircle} label="Safe Results" value={isLoading ? "—" : (stats?.safe ?? 0)} color="green" />
        <StatCard icon={TrendingUp} label="Suspicious" value={isLoading ? "—" : (stats?.suspicious ?? 0)} color="yellow" />
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="cyber-card p-5 lg:col-span-2">
          <div className="mb-4">
            <h2 className="font-display font-semibold text-text-primary text-sm">Scan Activity (7 days)</h2>
            <p className="text-text-secondary text-xs font-mono mt-0.5">Scans vs threats detected</p>
          </div>
          <ResponsiveContainer width="100%" height={180}>
            <AreaChart data={mockTrend}>
              ...
              <Tooltip
                contentStyle={{ background: "#0c1120", border: "1px solid #1a2540", borderRadius: "8px", fontFamily: "JetBrains Mono", fontSize: "12px" }}
        ...
        <div className="cyber-card p-5">
          <div className="mb-4">
            <h2 className="font-display font-semibold text-text-primary text-sm">Scan Breakdown</h2>
            <p className="text-text-secondary text-xs font-mono mt-0.5">Results distribution</p>
          </div>
          {pieData.some((d) => d.value > 0) ? (
            ...
            <div className="flex items-center justify-center h-36 text-text-secondary text-xs font-mono">
              No data yet
            </div>
          )}
        </div>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        {[
          ...
          <a key={item.label} href={item.href} className="cyber-card p-5 flex items-center gap-4 hover:border-neon-cyan/20 transition-all group">
            ...
            <div>
              ...
              <div className="text-text-secondary text-xs font-mono">{item.label}</div>
            </div>
            <div className="ml-auto text-text-secondary group-hover:text-neon-cyan transition-colors">→</div>
          </a>
        ))}
      </div>
    </div>
